...
#define AKM8963_RETRY_COUNT	5
#define AKM_INPUT_DELAY_MS_MIN		(50*1000*1000)
...
static int akm_vreg_dis(struct akm_compass_data *akm, int i)
{
	int err = 0;

	if (akm->vreg[i].ret && (akm->vreg[i].consumer != NULL)) {
		err = regulator_disable(akm->vreg[i].consumer);
		if (err)
			dev_err(&akm->i2c->dev, "%s %s ERR\n",
				__func__, akm->vreg[i].supply);
		else
			dev_dbg(&akm->i2c->dev, "%s %s\n",
				__func__, akm->vreg[i].supply);
	}
	akm->vreg[i].ret = 0;
	return err;
...
static int akm_vreg_dis_all(struct akm_compass_data *akm)
{
	unsigned int i;
	int err = 0;

	for (i = ARRAY_SIZE(akm_vregs); i > 0; i--)
		err |= akm_vreg_dis(akm, (i - 1));
	return err;
}

static int akm_vreg_en(struct akm_compass_data *akm, int i)
{
	int err = 0;

	if ((!akm->vreg[i].ret) && (akm->vreg[i].consumer != NULL)) {
		err = regulator_enable(akm->vreg[i].consumer);
		if (!err) {
			akm->vreg[i].ret = 1;
			dev_dbg(&akm->i2c->dev, "%s %s\n",
				__func__, akm->vreg[i].supply);
...
static int akm_vreg_en_all(struct akm_compass_data *akm)
{
	int i;
	int err = 0;

	for (i = 0; i < ARRAY_SIZE(akm_vregs); i++)
		err |= akm_vreg_en(akm, i);
	return err;
}

static void akm_vreg_exit(struct akm_compass_data *akm)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(akm_vregs); i++) {
		if (akm->vreg[i].consumer != NULL) {
			devm_regulator_put(akm->vreg[i].consumer);
			akm->vreg[i].consumer = NULL;
			dev_dbg(&akm->i2c->dev, "%s %s\n",
				__func__, akm->vreg[i].supply);
		}
	}
...
static int akm_vreg_init(struct akm_compass_data *akm)
{
	unsigned int i;
	int err = 0;

	for (i = 0; i < ARRAY_SIZE(akm_vregs); i++) {
		akm->vreg[i].supply = akm_vregs[i];
		akm->vreg[i].ret = 0;
		akm->vreg[i].consumer = devm_regulator_get(&akm->i2c->dev,
							  akm->vreg[i].supply);
		if (IS_ERR(akm->vreg[i].consumer)) {
			err |= PTR_ERR(akm->vreg[i].consumer);
			dev_err(&akm->i2c->dev, "%s err %d for %s\n",
				__func__, err, akm->vreg[i].supply);
			akm->vreg[i].consumer = NULL;
		} else {
			dev_dbg(&akm->i2c->dev, "%s %s\n",
				__func__, akm->vreg[i].supply);
...
static int akm_pm(struct akm_compass_data *akm, bool enable)
{
	int err;

	if (enable) {
		err = akm_vreg_en_all(akm);
	} else {
		err = akm_vreg_dis_all(akm);
	}
	if (err > 0)
		err = 0;
	if (err)
		dev_err(&akm->i2c->dev, "%s pwr=%x ERR=%d\n",
			__func__, enable, err);
	else
...
static void akm_pm_exit(struct akm_compass_data *akm)
{
	akm_pm(akm, false);
	akm_vreg_exit(akm);
}

static int akm_pm_init(struct akm_compass_data *akm)
{
	int err;
...
static long AKECS_SetMode_SngMeasure(struct akm_compass_data *akm)
{
	char buffer[2];
	...
	/* Set data */
	return akm_i2c_txdata(akm->i2c,buffer, 2);
}
static long AKECS_SetMode_SelfTest(struct akm_compass_data *akm)
{
	char buffer[2];
...
static long AKECS_SetMode_FUSEAccess(struct akm_compass_data *akm)
{
	char buffer[2];
	...
	struct akm_compass_data *akm, char *buf, int pos)
{
	...
	val = akm->delay[pos];
	mutex_unlock(&akm->val_mutex);
	printk("Ivan akm_compass_sysfs_delay_show [%d]= %lld\n",pos,val);
	...
	struct akm_compass_data *akm, char const *buf, size_t count, int pos)
{
	...
	mutex_lock(&akm->val_mutex);
	if (val < AKM_INPUT_DELAY_MS_MIN)
	  val = AKM_INPUT_DELAY_MS_MIN;
	akm->delay[pos] = val;
	mutex_unlock(&akm->val_mutex);
	printk("Ivan akm_compass_sysfs_delay_store [%d]= %lld\n",pos,val);
	return count;
}
...
int akm_compass_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("Ivan akm8963 akm_compass_probe\n");
...
//	s_akm->irq = client->irq;
	if (pdata->irq != 0)
	{
	  s_akm->irq = gpio_to_irq(pdata->irq);
...
{
	printk("AKM compass driver: release.");
