...
static unsigned int tpd_keycnt = TPD_KEY_COUNT;
...
static void synaptics_rmi4_swap_axis(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (!list_empty(&rmi->support_fn_list)) {
		list_for_each_entry(fhandler, &rmi->support_fn_list, link) {
			dev_dbg(&rmi4_data->i2c_client->dev,"function number 0x%x\n", fhandler->fn_number);
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	unsigned int key_y = TPD_BUTTON_Y_CENTER;
	...
	for (button = 0; button < f1a->valid_button_count; button++) {
		...
		//edit by Magnum 2013-11-07:enable virtual key vibrator
		#ifdef TPD_HAVE_BUTTON 
			if(button == 0)
				key_x = TPD_BUTTON1_X_CENTER; 
			else if(button == 1)
				key_x = TPD_BUTTON2_X_CENTER; 
			else if(button == 2)
				key_x = TPD_BUTTON3_X_CENTER;
	       #endif
		...
		bool enable)
{
	...
	if (enable) {
		...
		retval = synaptics_rmi4_i2c_read(rmi4_data,
				...
				&intr_status,
				rmi4_data->num_of_intr_regs);
		dev_dbg(&rmi4_data->i2c_client->dev,"Magnum register 0x14 == %d \n",intr_status);
		if (retval < 0)
			return retval;
...
static void create_virtual_key()
{
	...
	dev_err(&g_rmi4_data->i2c_client->dev,"failed to create board_properties\n");
}
#endif
...
{
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
	dev_dbg(&g_rmi4_data->i2c_client->dev,"Magnum  ctp suspend\n");
	dev_dbg(&rmi4_data->i2c_client->dev,"Magnum %s\n",__func__);
	const struct synaptics_dsx_platform_data *platform_data =
			rmi4_data->board;

	if (!rmi4_data->sensor_sleep) {
		dev_dbg(&g_rmi4_data->i2c_client->dev,"Magnum  sensor_sleep\n");
		wake_up(&rmi4_data->wait);
		synaptics_rmi4_irq_enable(rmi4_data, false);
		synaptics_rmi4_sensor_sleep(rmi4_data);
	}
