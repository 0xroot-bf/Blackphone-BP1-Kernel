 ...
 *
 */
static ssize_t akm_key_show(struct device *dev, struct device_attribute *attr,
			    char *buf)
{
	struct akm_inf *inf = dev_get_drvdata(dev);
	unsigned char *key;
	...
	printk("Ivan akm_key_show key=%s\n",inf->pdata.key);
	return sprintf(buf,
 ...
 */
static ssize_t akm_key_store(struct device *dev,
			     struct device_attribute *attr,
			     const char *buf, size_t count)
{
	struct akm_inf *inf = dev_get_drvdata(dev);
	...
	printk("Ivan akm_key_store key=%s\n",buf);
...
module_i2c_driver(ak8963_driver);
